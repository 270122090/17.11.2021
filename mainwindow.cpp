    ...
    , ui(new Ui::MainWindow)
{
    ...
    display_bookidetails();
    connect(ui->LoadiBookList, &QAction::triggered,this, &MainWindow::display_bookilist);
...
void MainWindow::display_bookilist()
{
    QFile inputFile("books.txt");
    inputFile.open(QIODevice::ReadOnly | QIODevice::Text);
    ...
    while(!in.atEnd())
    {
        ...
        //handle list of products ui
        ui->listBooks->addItem("     "+bookinfo.at(0)+"     " +bookinfo.at(1)+"   " + bookinfo.at(5));

                                               //Title          Author      DEWEY           ID              status          due date            image
        AllBooks* bookproduct = new AllBooks(bookinfo.at(0), bookinfo.at(1),bookinfo.at(2), bookinfo.at(3), bookinfo.at(4), bookinfo.at(5), bookinfo.at(6));
        booklist.push_back(bookproduct);
    }
    in.flush();
...
void MainWindow::issue_a_Book()
{
        //set the due date
            ...
            int memberindex =ui->listMember->currentRow();
                if (memberindex >=0)
                {
